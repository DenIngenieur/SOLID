 ...
 *
 * This example is the right way to do this, with multiple interfaces, one for each task.
 *
 * Notice that there is an interface PrinterTaskDuplex that is never used. It could be implemented when we also get
 * a newer machine that can print on both sides of a sheet. But the fact that we have an interface that covers this
 * feature does not cause any problems for the existing printers.
 */
